...
class Formula:
    ...
    def _parse_prefix(string: str) -> Tuple[Union[Formula, None], str]:
        ...
        if ch == '(':
            ...
            op = None
            for length in (3, 2, 1):
                if len(rest) >= length and is_binary(rest[:length]):
                    op = rest[:length]
                    rest = rest[length:]
                    break
            if op is None:
    ...
    def parse_polish(string: str) -> Formula:
        ...
        """
        # Optional Task 1.8
        def parse_prefix_polish(s: str) -> Tuple[Union[Formula, None], str]:
            if s == '':
                return None, ''
            ...
            if ch == '~':
                sub, rest = parse_prefix_polish(s[1:])
                if sub is None:
                    return None, rest
            ...
            op = None
            for length in (3, 2, 1):
                if len(s) >= length and is_binary(s[:length]):
                    op = s[:length]
                    rest = s[length:]
                    break
            if op is None:
                return None, ''
            left, rest = parse_prefix_polish(rest)
            if left is None:
                return None, rest
        ...
        parsed, rest = parse_prefix_polish(string)
        assert parsed is not None and rest == ''
        return parsed
